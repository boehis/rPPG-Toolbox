...
from neural_methods.model.AriaNet import *
from neural_methods.trainer.BaseTrainer import BaseTrainer
from torch.autograd import Variable
from tqdm import tqdm


class AriaNetTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        self.model = AriaNet_Multimodal(
            frames=config.MODEL.ARIANET.FRAME_NUM,
            integration_strategy=config.MODEL.ARIANET.INTEGRATION_STRATEGY, 
            integration_layers=config.MODEL.ARIANET.INTEGRATION_LAYERS, 
            sensor_type=config.MODEL.ARIANET.SENSOR_TYPE,
            skip_connections=config.MODEL.ARIANET.SKIP_CONNECTIONS,
            ).to(self.device)
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            ...
            self.loss_model_train = get_loss(config.MODEL.ARIANET.LOSS_TRAIN)
            self.loss_model_val = get_loss(config.MODEL.ARIANET.LOSS_TRAIN)
            ...
            raise ValueError("AriaNet trainer initialized in incorrect toolbox mode!")
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                rPPG = self.model(
                    ...
                    batch[4].to(torch.float32).to(self.device),
                    batch[5].to(torch.float32).to(self.device),
                )
        ...
        with torch.no_grad():
            ...
            for valid_idx, valid_batch in enumerate(vbar):
                ...
                rPPG = self.model(
                    ...
                    valid_batch[4].to(torch.float32).to(self.device),
                    valid_batch[5].to(torch.float32).to(self.device),
                )
                rPPG = (rPPG - torch.mean(rPPG)) / torch.std(rPPG)  # normalize
                BVP_label = (BVP_label - torch.mean(BVP_label)) / \
                            torch.std(BVP_label)  # normalize
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            for _, test_batch in enumerate(tqdm(data_loader["test"], ncols=80)):
                ...
                pred_ppg_test = self.model(
                    test_batch[0].to(self.config.DEVICE),
                    test_batch[4].to(self.config.DEVICE),
                    test_batch[5].to(self.config.DEVICE),
                )
